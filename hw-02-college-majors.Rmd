...
Because it isn't affected by outliers that might throw off the average.
...
```
A binwidth of 5000 seems to be better, as 1000 makes the ggplot hard to understand.
...
ggplot(college_recent_grads, aes(x = sharewomen, y = median, color = major_type)) +
  geom_point()
```
This scatterplot displays the relationship between the share of women in a major and median income, differentiated by STEM and non-STEM fields.

## Further exploration

10. Ask a question of interest to you, and answer it using summary statistic(s) and/or visualization(s).

How many people earn in the lowest and the highest of the 25th percentile, and what do they earn?

```{r}
ggplot(college_recent_grads, aes(x = p25th, y = median)) +
  geom_point() +
  labs(title = "",
       y = "",
